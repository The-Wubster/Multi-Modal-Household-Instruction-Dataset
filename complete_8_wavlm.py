...
def extract_speech_features(wav_fn, cmvn=True):
    ...
    features = layer_reps[layer_to_access].detach().numpy()"""
...
for idx, curr_query in tqdm(enumerate(query_mfcc)):
    ...
    # Applying rules to improve final prediction results:
...
# Saving results to an output folder to be analysed:
with open('output_metrics.csv', 'a') as output_file:
    writer_object = csv.writer(output_file)
    writer_object.writerow(["wavlm_seq_encodde_1024", "eng_105", n_shot, n_nearest, final_accs[0][0], final_accs[0][1], final_accs[0][2], final_accs[1][0], final_accs[1][1], final_accs[1][2], final_accs[2][0], final_accs[2][1], final_accs[2][2], restrict[0], restrict[1], restrict[2], restrict[3], restrict[4], object_not_present])
    output_file.close()