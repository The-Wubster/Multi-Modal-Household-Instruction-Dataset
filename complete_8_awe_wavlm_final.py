...
def extract_speech_features(wav_fn, cmvn=True):
    ...
    assert sr == 16000
    # Extract last layer representation:
    if cfg.normalize:
        wav_input_16khz = torch.nn.functional.layer_norm(wav , wav.shape)
    rep = model.extract_features(wav_input_16khz)[0]
    features = rep.detach()

    # Extract representation of specific encoder layer:
    """if cfg.normalize:
        wav_input_16khz = torch.nn.functional.layer_norm(wav , wav.shape)
    ...
    features = layer_reps[layer_to_access].detach()"""
...
if num_classes == 105:
    ...
    query_recordings_folder = "./instruction_files/reduced_recordings_eng/test/"
    query_list = label_audio_as_tuple(query_recordings_folder)
    train_recordings_folder = "./instruction_files/reduced_recordings_eng/train/"
    train_list = label_audio_as_tuple(train_recordings_folder)
...
if unseen == False:
    ...
    final_accs = cm.return_accuracies_complete(remapped_classes, predicted_classes, indices_to_remove, len(query_mfcc), num_classes)
    print("=================================================================================================")
    print("Prediction Accuracy for " + str(n_nearest) + " Nearest Neigbours:")
    print("-------------------------------------------------------------------------------------------------")
...
with open('output_metrics.csv', 'a') as output_file:
    if unseen == True:
        ...
        writer_object.writerow(["wavlm_awe_eng_out", "eng_105", n_shot, n_nearest, final_accs[0][0], final_accs[0][1], final_accs[0][2], final_accs[1][0], final_accs[1][1], final_accs[1][2], final_accs[2][0], final_accs[2][1], final_accs[2][2], restrict[0], restrict[1], restrict[2], restrict[3], restrict[4], object_not_present])
        output_file.close()